...
if __name__ == '__main__':
    ...
    comp6741CourseDescription = ("Knowledge representation and reasoning. Uncertainty and conflict resolution. "
                                 "Design of intelligent systems. Grammar-based, rule-based, and blackboard architectures. "
                                 "A project is required.")

    comp6721CourseDescription = ("The course covers heuristic and adversarial searches for concrete applications. "
                                 "It then discusses automated reasoning, advanced knowledge representation and dealing "
                                 "with uncertainly for Artificial Intelligence applications. Finally, it introduces "
                                 "autoencoders, recurrent neural networks and sequence to sequence models. "
                                 "A project is required.")
    ...
    for index, row in readCsv.iterrows():
        ...
        if not pd.isnull(row['Long Title']):
            g.add((URIRef(FOCUDATA + (row['Subject'] + str(row['Catalog']))), FOCU.courseName,
                   Literal(row['Long Title'])))
        if not pd.isnull(row['Subject']):
            g.add((URIRef(FOCUDATA + (row['Subject'] + str(row['Catalog']))), FOCU.courseSubject,
                   Literal(row['Subject'])))
        if not pd.isnull(row['Catalog']):
            g.add(
                (URIRef(FOCUDATA + (row['Subject'] + str(row['Catalog']))), FOCU.courseNumber, Literal(row['Catalog'])))
        if row['Subject'] + str(row['Catalog']) == "COMP6721":
            if not pd.isnull(row['Description']):
                g.add((URIRef(FOCUDATA + (row['Subject'] + str(row['Catalog']))), FOCU.description,
                       Literal(comp6721CourseDescription)))
        elif row['Subject'] + str(row['Catalog']) == "COMP6741":
            if not pd.isnull(row['Description']):
                g.add((URIRef(FOCUDATA + (row['Subject'] + str(row['Catalog']))), FOCU.description,
                       Literal(comp6741CourseDescription)))
        else:
            if not pd.isnull(row['Description']):
                g.add((URIRef(FOCUDATA + (row['Subject'] + str(row['Catalog']))), FOCU.description,
        ...
        if not pd.isnull(row['Class Units']):
            g.add((URIRef(FOCUDATA + (row['Subject'] + str(row['Catalog']))), FOCU.courseCredits,
                   Literal(str(row['Class Units']))))
        ...
        try:
            for row in csv_reader:
                ...
                if row[1] == "COMP" and row[2] == "6721":
                    ...
                    #           ADD TOPIC
                    # -----------------------------------------------------
                    topics = open("saveTopics.txt").readlines()
                    for topic in topics:
